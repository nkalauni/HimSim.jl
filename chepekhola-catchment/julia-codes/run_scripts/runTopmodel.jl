...
@variables ζt Suz(t) Ssz(t)
@variables Qex(t) ζcrit(t) Ac(t) Peff(t) Ea(t) Qv(t) Qb(t) Qof(t) Q(t)
@parameters Suzmax St Kd q0 f χ ϕ

D = Differential(t)
Iζ = Integral(ζ in DomainSets.ClosedInterval(ζcrit, Inf))

a = Iζ(1 / (χ * gamma(ϕ)) * (max(t - μ, 0) / χ)^(ϕ - 1) * exp(-(max(t - μ, 0) / χ))) |> collect
a = Iζ(1 / (χ * gamma(ϕ)) * ((ζ - μ) / χ)^(ϕ - 1) * exp(-((ζ - μ) / χ)))
...
eqs = [
    ζcrit ~ f * Ssz + χ * ϕ + μ,
    Ac ~ Integral(ζ in DomainSets.ClosedInterval(ζcrit, Inf))(1 / (χ * gamma(ϕ)) * (max(t - μ, 0) / χ)^(ϕ - 1) * exp(-(max(t - μ, 0) / χ))),
    Peff ~ P(t) * (1 - Ac),
